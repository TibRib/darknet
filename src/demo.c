...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes, int avgframes,
    ...
    int benchmark, int benchmark_layers, char* out_json)
{
    ...
    //JSON File initialization if needed
    char *json_buf = NULL;
    int json_image_id = 0;
    FILE* json_file = NULL;
    if (out_json) {
        json_file = fopen(out_json, "wb");
        if(!json_file) {
          error("fopen failed");
        }
        char *tmp = "[\n";
    ...
    while(1){
        ++count;
        {
            ...
            if (json_file) {
                ...
                ++json_image_id;
                json_buf = detection_to_json(dets, nboxes, l.classes, names, json_image_id, filename);
